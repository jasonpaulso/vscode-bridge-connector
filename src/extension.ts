...
async function startServer() {
  ...
  server = http.createServer(async (req, res) => {
    ...
    console.log(
      `[${timestamp}] ${req.method} ${req.url} - ${userAgent} from ${req.socket.remoteAddress}`
    );
    ...
    if (req.method === "POST") {
      ...
      if (contentLength > 10000) {
        // 10KB limit
        console.warn(
          `[${timestamp}] Request too large: ${contentLength} bytes`
        );
    ...
    if (key !== apiKey || !key) {
      console.warn(
        `[${timestamp}] Unauthorized access attempt with key: ${
          key ? "INVALID" : "MISSING"
        }`
      );
    ...
    if (req.method === "POST" && req.url === "/command") {
      ...
      req.on("end", async () => {
        try {
          ...
          console.log(
            `[${timestamp}] Executing command: ${command} with args:`,
            args || []
          );
          ...
          if (command === "vscode.languages.getDiagnostics") {
            const pathUri = vscode.Uri.file(args?.[0] || "");
            const result = vscode.languages.getDiagnostics(pathUri);
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                result,
  ...
  server.listen(port, "127.0.0.1", () => {
    updateStatus();
    console.log(
      `[${new Date().toISOString()}] Bridge Connector started on port ${port}`
    );
